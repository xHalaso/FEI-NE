...
class Population():
    ...
    def crossGenomes(self):
        ...
        if ((self.debug) and (self.debugL==4)): oldpop=np.copy(newpop); print(oldpop)
    ...
    def globalMut(self):
        ...
        if ((self.debug) and (self.debugL==4)): print("Compare after GlobalMut:\n", oldpop == np.array([gen.gen for gen in self.genomes]),f"\n {np.array([gen.gen for gen in self.genomes])}")
